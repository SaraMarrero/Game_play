<?php
	...
	if(!$_GET){
		header('Location: /juegos/Play Station 5?pagina=1');
	}
...
<section class="bg-gradient-to-r from-gray-900 via-blue-800 to-gray-900">
	<main class="w-3/5 m-auto bg-blue-950">
		<h1 class="text-center p-5 font-bold text-3xl text-gray-200">Play Station 5</h1>
		...
		<div class="flex-1 flex justify-center mr-auto py-5">
			<nav class="content-center">
				<ul class="inline-flex -space-x-px">
					...
					<li>
						<a href="/juegos/Play Station 5.php?pagina=<?php echo $_GET['pagina'] <= 1 ? $_GET['pagina'] : $_GET['pagina']; ?>"
					...
					<?php for($i = 1; $i <= $paginas; $i++){ ?>
						<li <?php echo $_GET['pagina'] == $i ? 'class="font-bold text-purple-900"' : 'class="text-gray-500"' ?>>
							<a href="/juegos/Play Station 5?pagina=<?php echo $i ?>"
								class="bg-white border border-gray-300 leading-tight py-2 px-3"><?php echo $i ?>
							</a>
						</li>
					<?php } ?>
					<!-- Veo en que página estoy si es mayor o igual que 3 se queda en la misma página, si no se suma 1 -->
					<li>
						<a href="/juegos/Play Station 5?pagina=<?php echo $_GET['pagina'] >= $paginas ? $_GET['pagina'] : $_GET['pagina']+1; ?>"
